...
Figure~\ref{fig:pipeline} shows the closed-loop design. The UAV receives aligned RGB and long-wave infrared feeds, fuses them in a cross-attention module~\cite{vaswani2017attention}, and estimates target geometry and distress-state evidence. The detections are projected into geographic coordinates using UAV telemetry. A drift-aware Kalman filter maintains target tracks and inflates measurement covariance when observations are uncertain. If a target has high uncertainty and non-negligible expected risk, the UAV temporarily descends to collect a clearer view. Confirmed tracks are passed to a priority engine that schedules rescue assets.
