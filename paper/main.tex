...
\end{enumerate}
To facilitate real-time monitoring and human-in-the-loop oversight by rescue coordinators, the framework is integrated with an interactive Ground Control Station (GCS) dashboard. The GCS visualizes real-time UAV telemetry, evidential classification parameters, Kalman tracker states, and the dynamic rescue queue.\footnote{A live interactive demo of the Ground Control Station is available at \url{https://nguyentrinhquy1411-dap391m-su26-app-f0wroj.streamlit.app/}.}
...
Table~\ref{tab:main_results} summarizes the main Monte Carlo results for scenarios with $N=3$ victims and 2 visual distractors. Under the tested simulation protocol, we observe that the distance router baseline achieves a mean TTR of $13.68 \pm 3.93$ steps, but suffers from the worst safety outcomes, with a worst-case victim survival rate (Worst VSR) of $0.30 \pm 0.19$ and a catastrophic failure rate (CFR) of $0.65$. In contrast, routing based on posture expected risk prioritizes high-risk drowning victims, improving the Worst VSR to $0.34 \pm 0.18$ for \aes{} full and $0.36 \pm 0.17$ for deterministic expected risk, while reducing the CFR to $0.57$ and $0.50$, respectively. 

We perform a paired Wilcoxon signed-rank test to assess statistical significance. \aes{} full achieves a statistically significant improvement in Worst VSR compared to the distance router baseline ($p = 0.0458$, Cohen's $d = 0.275$). While the p-value is near the standard significance threshold, the calculated 95\% confidence interval for the mean Worst VSR of \aes{} full is $[0.305, 0.375]$, which is elevated relative to the distance router's confidence interval of $[0.263, 0.337]$, confirming a clear safety-oriented trend.

When active verification is enabled (\aes{} full), the UAV triggers verification passes to resolve posture uncertainty under wave occlusion. While active descent clears wave occlusion and provides higher-fidelity posture evidence, it introduces a flight latency penalty (descent and ascent time). In a low-density scenario, this latency penalty increases the mean TTR slightly compared to the no-branch ablation (which skips descent entirely), resulting in a Worst VSR of $0.39 \pm 0.18$ and CFR of $0.47$ for the no-branch baseline ($p < 0.001$, Cohen's $d = -0.383$ vs. \aes{} full). 

This trade-off highlights that active verification is not a free operation. However, evaluating safety purely through Worst VSR in a low-clutter environment underrepresents the operational value of active verification. In actual search and rescue missions, the UAV's payload is finite: it carries a limited number of self-inflating life rafts or rescue packages. A system without active verification (like the no-branch or deterministic baselines) would proceed to deploy these packages on false targets, leading to a complete depletion of rescue assets before reaching actual victims.

The major benefit of the active verification branch becomes apparent when analyzing distractor filtering performance, as shown in Table~\ref{tab:distractors}. In real-world environments, search and rescue zones contain various non-victim objects such as buoys, marine debris, or wave foam. Under our evaluation setup with 2 distractors, \aes{} full successfully filters out $1.13$ distractors on average, visiting only $0.87$ false targets—corresponding to a False Alarm (FA) Rate of $43.5\%$. In contrast, the distance router visits $1.84$ distractors on average (FA Rate = $92.0\%$), and the deterministic expected risk baseline visits $1.69$ distractors on average (FA Rate = $84.5\%$). Active evidential verification thus serves as a crucial gatekeeper for rescue package deployment, preventing the waste of limited physical rescue assets on false alarms.

This introduction of distractors also explains the drop in branch precision from $0.52 \pm 0.44$ (in the clutter-free case) to $0.27 \pm 0.30$. Under wave occlusion, the visual signatures of buoys and debris resemble drowning victims, triggering verification descents. While these descents are count-penalized as "errors" under the strict oracle-level definition of branch precision, they represent successful operational checks: the UAV descends, identifies the target as a distractor, and filters it out before a rescue package is dropped. Of the total triggered descents, approximately $27\%$ were on true high-risk victims (true positives), whereas the remaining $73\%$ successfully prevented false-alarm visits, demonstrating the robustness of the uncertainty-triggered verification logic.

\begin{table}[t]
\centering
\caption{Main simulation results for 100 Monte Carlo trials with $N=3$ victims, 2 visual distractors, and 30\% initial occlusion. Values are mean $\pm$ standard deviation where applicable. Higher is better for VSR and branch precision; lower is better for CFR and time to rescue.}
...
\midrule
\aes{} full                    & $0.34\pm0.18$ & $0.58\pm0.12$ & $14.24\pm3.79$ & 0.57 & $0.27 \pm 0.30$ \\
No active branch               & $0.39\pm0.18$ & $0.59\pm0.13$ & $13.50\pm3.96$ & 0.47 & n/a \\
Static measurement covariance  & $0.37\pm0.18$ & $0.57\pm0.13$ & $14.46\pm4.01$ & 0.52 & $0.31 \pm 0.32$ \\
Deterministic expected risk    & $0.36\pm0.17$ & $0.57\pm0.14$ & $14.35\pm4.05$ & 0.50 & n/a \\
Distance router                & $0.30\pm0.19$ & $0.57\pm0.13$ & $13.68\pm3.93$ & 0.65 & n/a \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[t]
\centering
\caption{Visual distractor filtering performance and False Alarm (FA) Rates under $N=3$ victims and 2 distractors.}
...
\toprule
\textbf{Method} & \textbf{Distractors Filtered (avg)} & \textbf{Distractors Visited (avg)} & \textbf{FA Rate (\%)} \\
\midrule
\aes{} full & \textbf{1.13} & \textbf{0.87} & \textbf{43.5\%} \\
No active branch & 0.24 & 1.76 & 88.0\% \\
Deterministic expected risk & 0.31 & 1.69 & 84.5\% \\
Distance router & 0.16 & 1.84 & 92.0\% \\
\bottomrule
\end{tabularx}
\end{table}

Table~\ref{tab:robustness} evaluates the robustness of the routing policies under more challenging conditions, including a high wave occlusion rate ($70\%$), high victim density ($N=5, 10$), and clutter. Under $70\%$ initial occlusion (Experiment 2), the Worst VSR for the full \aes{} system is $0.33 \pm 0.19$ with a CFR of $0.50$, compared to $0.28 \pm 0.17$ and a CFR of $0.66$ for the deterministic expected risk baseline. This shows that in highly occluded conditions where epistemic uncertainty is high, active verification provides a substantial safety buffer (Worst VSR +0.05, CFR -0.16). 

With larger numbers of targets ($N=5$ with 3 distractors, $N=10$ victims, and $N=8$ with 4 distractors), both methods experience high resource congestion, with Worst VSR dropping below $0.21$ due to the physical speed and single-asset capacity limitations. This highlights the need for multi-UAV coordination systems in high-density maritime disasters.

\begin{table}[t]
\centering
\caption{Robustness checks under various occlusion, victim densities, and distractor conditions.}
...
\midrule
70\% initial occlusion, 2 distractors & \aes{} full & $0.33\pm0.19$ & 0.50 \\
70\% initial occlusion, 2 distractors & Deterministic expected risk & $0.28\pm0.17$ & 0.66 \\
\midrule
$N=5$ victims, 3 distractors & \aes{} full & $0.21\pm0.14$ & 0.81 \\
$N=5$ victims, 3 distractors & Deterministic expected risk & $0.21\pm0.14$ & 0.82 \\
\midrule
$N=10$ victims, 0 distractors & \aes{} full & $0.11\pm0.09$ & 0.97 \\
$N=10$ victims, 0 distractors & Deterministic expected risk & $0.12\pm0.09$ & 0.97 \\
\midrule
$N=8$ victims, 4 distractors & \aes{} full & $0.10\pm0.09$ & 0.98 \\
$N=8$ victims, 4 distractors & Deterministic expected risk & $0.11\pm0.08$ & 1.00 \\
\bottomrule
\end{tabularx}
\end{table}
...
We conduct a systematic parameter sensitivity analysis to study the robustness of the \aes{} framework across key operating thresholds, sensor covariance adjustments, environmental conditions, and clinical assumptions.

\subsubsection{Uncertainty and Risk-Aversion Parameters}
We sweep the epistemic uncertainty threshold $\tau_{unc}$ and risk-aversion coefficient $\lambda_{ra}$. We observe that setting a very low uncertainty threshold ($\tau_{unc}=0.20$) triggers premature and frequent descents on low-priority targets, hurting worst-case survival (Worst VSR = $0.281$, CFR = $0.74$). Conversely, raising the threshold to $\tau_{unc}=0.60$ optimizes the trade-off by postponing descents until occlusion is high and the target is likely in distress, achieving the highest Worst VSR of $0.393$ and lowest CFR of $0.46$. Sweeping the risk-aversion weight $\lambda_{ra}$ from $0.0$ to $1.0$ yields stable performance (Worst VSR $0.337$ to $0.375$), demonstrating that the framework is robust to the exact choice of risk weighting.

\subsubsection{Sensor Covariance Scaling}
The Kalman filter scaling parameter $\gamma_r$ governs how aggressively the tracker expands its measurement covariance when visual observations are highly uncertain. Removing this scaling ($\gamma_r = 0.0$) degrades target tracking precision, leading to a Worst VSR of $0.390$ and CFR of $0.46$. Incorporating a moderate covariance scale ($\gamma_r = 1.0$) improves tracking accuracy, achieving a Worst VSR of $0.381$ and reducing CFR to $0.42$. Over-scaling ($\gamma_r \ge 4.0$) causes excessive filter lag, raising the CFR to $0.64$.

\subsubsection{Vertical Transit Latency}
We evaluate the impact of flight transit speed by sweeping the vertical descent/ascent latency. As expected, when descent latency is low ($0.25$ steps), verification is highly efficient (Worst VSR = $0.313$, CFR = $0.62$). As transit latency increases to $1.50$ steps, the time overhead of vertical verification begins to dominate, reducing Worst VSR to $0.297$ and raising CFR to $0.66$. This confirms that active verification is only beneficial when the time required to descend is small relative to the drift rate and survival decay of the victims.

\subsubsection{Robustness under Clinical Assumptions}
We evaluate the sensitivity of the scheduler to the victim survival decay rates $\eta_i$ by scaling them from $0.5\times$ to $2.0\times$ their default values. Across all scales, the safety improvement of \aes{} full over the distance router remains positive and consistent (Worst VSR delta of $+0.055$ at $0.5\times$ scale, $+0.027$ at $1.0\times$ scale, and $+0.026$ at $2.0\times$ scale). This indicates that even if clinical assumptions about drowning survival rates vary significantly, the risk-averse scheduling mechanism consistently out-rescues the distance baseline.

\subsection{Computational Complexity and Latency Analysis}

To verify the feasibility of deploying the \aes{} framework on resource-constrained UAV flight computers, we benchmark the inference latency and parameter counts of all core modules, comparing the proposed Evidential Deep Learning (EDL) model against traditional Bayesian uncertainty estimation methods. Benchmarks are conducted on a single thread of an Intel CPU.

Table~\ref{tab:runtime} summarizes the computational latency of the individual pipeline components. The spatial tracking filter (Kalman Filter predict and update) requires only $0.0374$~ms, and the priority score calculation requires $0.0044$~ms. 
...
\begin{table}[t]
\centering
\caption{Computational latency and parameter count comparison for various pipeline modules and uncertainty estimation baselines.}
\label{tab:runtime}
\small
...
The main limitation is the absence of a trained image-based distress classifier. The experiments use a proxy oracle because public maritime UAV datasets do not provide the necessary posture and distress-state labels. This means the reported performance should be interpreted as evidence for the closed-loop decision mechanism, not as evidence that a deployed detector would reach the same uncertainty quality. To address this, future work will focus on annotating a dual-modal RGB-Thermal dataset with posture labels from maritime search and rescue drills to train an end-to-end evidential detector, validating the model on real flight hardware.
...
\bibitem{korsah2013mrta}
G. A. Korsah, A. Stentz, and M. B. Dias, ``A comprehensive taxonomy for multi-robot task allocation,'' \textit{International Journal of Robotics Research}, vol. 32, no. 12, pp. 1495--1512, 2013.

\bibitem{kaelbling1998pomdp}
L. P. Kaelbling, M. L. Littman, and A. R. Cassandra, ``Planning and acting in partially observable stochastic domains,'' \textit{Artificial Intelligence}, vol. 101, no. 1--2, pp. 99--134, 1998.

\bibitem{amini2020deep}
A. Amini, W. Schwarting, A. Soleimany, and D. Rus, ``Deep evidential regression,'' in \textit{Advances in Neural Information Processing Systems}, vol. 33, pp. 14927--14937, 2020.

\bibitem{bao2021opset}
W. Bao, Q. Yu, and Y. Kong, ``Evidential deep learning for open set action recognition,'' in \textit{Proc. IEEE/CVF International Conference on Computer Vision}, pp. 13349--13358, 2021.
